...
public class DishServiceImpl implements DishService {
    ...
    public void startOrStop(Integer status, Long id) {
        ...
        dishMapper.update(dish);    // 只会根据id更新dish的status，其它字段和原数据库的一样
        ...
        if (status == StatusConstant.DISABLE) {
            ...
            if (setmealIds != null && !setmealIds.isEmpty()) {
                for (Long setmealId : setmealIds) { // 停售套餐
                    Setmeal setmeal = Setmeal.builder()
                            .id(setmealId)
                            .status(status)
                            .build();
                    setmealMapper.update(setmeal);  // 根据套餐id更新套餐的数据
                }
            }

        }

        if (dish.getStatus() == StatusConstant.DISABLE && status == StatusConstant.ENABLE) {    // 停售中的菜品启售
            dish.setStatus(StatusConstant.ENABLE);
            dishMapper.update(dish);
        } else if (dish.getStatus() == StatusConstant.ENABLE && status == StatusConstant.DISABLE) { // 启售中的菜品，准备停售
            // 若启售的菜品，关联了套餐就不能停售
            List<Long> dishIds = new ArrayList<>();
            dishIds.add(id);
            List<Long> setmealIds = setmealDishMapper.getSetmealIdsByDishIds(dishIds);
